...
public class FundTransferActivity extends AppCompatActivity {
    ...
    String selectedPaymentMode = "Select Payment Mode", selectedBankName = "select", selectedBankId;

    EditText etAmount, etRefrenceNumber, etRemarks, etDepositBYBank;
    TextView tvDate, tvUploadReceipt;
    Button btnSubmit;
    SharedPreferences sharedPreferences;
    String userId;
    ArrayList<String> bankList,bankIdList;
    String amount, comment, bankRefNo, date = "select", depositByBank;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        activityTitle.setText("Fund Request");
        ...
        btnSubmit.setOnClickListener(v -> {
            if (checkInternetState()) {
                if (checkInputs()) {
                    ...
                    depositByBank = etDepositBYBank.getText().toString().trim();
                    doPaymentRequest();
                }
            } else {
    ...
    private void doPaymentRequest() {
        ...
        Call<JsonObject> call = RetrofitClient.getInstance().getApi().doPaymentRequest("AUTH_KEY", userId, amount, selectedPaymentMode, selectedBankId
                , selectedBankName, depositByBank, bankRefNo, comment, fileUrl);
            ...
            public void onResponse(Call<JsonObject> call, Response<JsonObject> response) {
                if (response.isSuccessful()) {
                    ...
                    try {
                        ...
                        String transactions = jsonObject1.getString("message");
                    ...
                    if (!TextUtils.isEmpty(etRemarks.getText())) {
                        if (!date.equalsIgnoreCase("select")) {
                            if (!selectedBankName.equalsIgnoreCase("select")) {
                                if (!TextUtils.isEmpty(etDepositBYBank.getText())) {

                                    return true;
                                } else {
                                    etDepositBYBank.setError("Required");
                                    return false;
                                }
                            } else {
    ...
    private void serverUpload(File myfile) {
        ...
        Call<JsonObject> call = RetrofitClient.getInstance().getApi().uploadfile(body);
        call.enqueue(new Callback<JsonObject>() {
    ...
    private void inhitViews() {
        ...
        etDepositBYBank = findViewById(R.id.et_deposit_by_bank);
    }
    ...
    private void getBankList() {
        ...
        Call<JsonObject> call = RetrofitClient.getInstance().getApi().getBankList("AUTH_KEY", userId);
        call.enqueue(new Callback<JsonObject>() {
            @Override
            public void onResponse(Call<JsonObject> call, Response<JsonObject> response) {
                if (response.isSuccessful()) {
