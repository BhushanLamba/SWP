...
public class CreditDebitBalanceActivity extends AppCompatActivity {
    ...
    private void checkCreditDebit() {
        ...
        if (title.equalsIgnoreCase("Credit Balance")) {
            call = RetrofitClient.getInstance().getApi().doCreditBalance("",userId,selectedUserId,amount,remarks,"cr");
            doCreditDebit();
        } else if (title.equalsIgnoreCase("Debit Balance")) {
            call = RetrofitClient.getInstance().getApi().doCreditBalance("",userId,selectedUserId,amount,remarks,"dr");
            doCreditDebit();
        }
    }
                ...
                if (response.isSuccessful()) {
                    if (response.isSuccessful()) {
                        ...
                        try {
                            ...
                            String responseMessage = jsonObject1.getString("message");
    ...
    private void getUsers() {
        ...
        Call<JsonObject> call = RetrofitClient.getInstance().getApi().getUsers("AUTH_KEY",userId);
        call.enqueue(new Callback<JsonObject>() {
            @Override
            public void onResponse(Call<JsonObject> call, Response<JsonObject> response) {
                if (response.isSuccessful()) {
